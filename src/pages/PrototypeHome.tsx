...
const options: { id: Page; code: string; title: string; subtitle: string; description: string }[] = [
  ...
  { id: 'c', code: 'C', title: 'AI Support Intervention', subtitle: 'Xem đề xuất hỗ trợ của AI', description: 'Xem học viên AI đề xuất, lý do và tin nhắn xác nhận trước khi gửi.' },
]

export function PrototypeHome({ onSelect }: { onSelect: (page: Page) => void }) {
  return (
    <main className="container home">
      <PrototypeHeader eyebrow="MICRO-PROTOTYPE" title="Chọn một phương án" description="Bạn là Lab Coach đang hỗ trợ 50 học viên trong một buổi thực hành. Hãy dùng từng phương án để xác định ai nên được ưu tiên hỗ trợ và quyết định hành động tiếp theo." />
      <section className="context-card" aria-label="Bối cảnh phiên học">
        <div><span>Vai trò</span><strong>Lab Coach</strong></div>
